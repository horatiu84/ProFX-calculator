...
const WeeklySchedule = () => {
  ...
  const specialEvents = {
    0: [{ name: "Webinar începători cu Sergiu și John", time: "20:00", duration: 1 }], // Luni
  ...
  const EventCard = ({ event, dayIndex, isWebinar = false }) => {
    ...
    return (
      <div className={`relative bg-slate-800 rounded-lg border transition-all duration-300 hover:scale-105 ${
        status === 'passed' 
          ...
          <div className="flex justify-between items-start mb-2 md:mb-3">
            <div className="flex-1">
              ...
              <div className="flex flex-col md:flex-row md:items-center md:space-x-2 space-y-1 md:space-y-0 mb-1 md:mb-2">
                <div className={`px-2 md:px-3 py-1 rounded-full text-xs md:text-sm font-medium w-fit ${
                  status === 'passed' 
              ...
              <p className={`text-xs ${status === 'passed' ? 'text-gray-500' : 'text-gray-400'}`}>
                Durată: {duration} {duration === 1 ? 'oră' : 'ore'}
              </p>
            </div>
            
            <div className={`px-2 md:px-3 py-1 rounded-full text-xs font-bold uppercase ml-2 ${
              status === 'live' 
              ...
              {status === 'live' ? '🔴 LIVE' : status === 'passed' ? 'TRECUT' : 'PROGRAMAT'}
            </div>
          </div>
        </div>
      </div>
    ...
    if (dayIndex === 5 || dayIndex === 6) {
      return (
        <div className="flex flex-col items-center justify-center h-48 md:h-64 text-center">
          ...
          <h2 className="text-xl md:text-2xl font-bold text-yellow-400 mb-1 md:mb-2">RELAXARE</h2>
          <p className="text-gray-400 text-sm md:text-base">Weekend de odihnă și reîncărcare</p>
        </div>
      );
    }

    // Zilele de lucru (Luni-Vineri)
    const regularEvents = weekdayEvents.map(event => ({ ...event, isWebinar: false }));
    const webinarEvents = (specialEvents[dayIndex] || []).map(event => ({ ...event, isWebinar: true }));
    
    const allEvents = [...regularEvents, ...webinarEvents]
    ...
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <div className="bg-slate-800 border-b border-slate-700">
        <div className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-8 text-center">
          <h1 className="text-2xl md:text-4xl font-bold text-yellow-400 mb-2">PROGRAM SĂPTĂMÂNAL</h1>
          <p className="text-gray-400 mb-1 text-sm md:text-base">Învață să tranzacționezi gratuit, de la zero</p>
          <p className="text-xs md:text-sm text-gray-500">
            Astăzi este {daysOfWeek[new Date().getDay() === 0 ? 6 : new Date().getDay() - 1]}, 
            {currentDateTime.toLocaleDateString('ro-RO')} - {currentDateTime.toLocaleTimeString('ro-RO', { hour: '2-digit', minute: '2-digit' })}
          </p>
        </div>
      </div>
            ...
            <div className="relative">
              ...
              >
                <div className="flex items-center space-x-3">
                  <span className="font-semibold text-yellow-400">{daysOfWeek[activeTab]}</span>
                  {activeTab === (new Date().getDay() === 0 ? 6 : new Date().getDay() - 1) && (
                    ...
                    return (
                      <button
                        ...
                        className={`w-full px-4 py-3 text-left hover:bg-slate-700 transition-colors duration-200 flex items-center justify-between ${
                          activeTab === index ? 'bg-slate-700 text-yellow-400' : 'text-white'
                        }`}
                      >
                        <div className="flex items-center space-x-3">
